...
auto* randomname(list_input_v) {
  ...
  op_params.axis = axis; 
  op_params.inputs_count = tensor_num; 
  // auto* cpu_backend_context = new CpuBackendContext();
  // cpu_backend_context->SetMaxNumThreads(-1);   
  optimized_ops::Concatenation(op_params, all_shape_ptr_.data(),            \
                                all_data_.data(), RuntimeShape(output_dims_size, output_dims_raw), \
                                output_data);    
